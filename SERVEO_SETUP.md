...
### OAuth: `client_id: Field required` при первом подключении

На **бесплатном** Serveo при первом заходе в браузере показывается одноразовая страница-предупреждение «you are about to visit…». Она теряет query-параметры у ссылки `/authorize`, поэтому вместо страницы `/consent` можно увидеть ошибку `client_id / response_type / code_challenge: Field required`. Это **не** сбой сервера.

- Нажмите в браузере **«Назад»** и откройте ссылку авторизации ещё раз (или повторите *Connect* в клиенте) — предупреждение уже снято на эту сессию браузера, и откроется нормальная страница `/consent`.
- Начиная с 2.1.0 сервер на этот случай отдаёт понятную страницу-подсказку вместо сырого JSON.
- Чтобы интерстициал не появлялся вовсе, используйте свой домен (`public_url`) или платный аккаунт Serveo с зарезервированным hostname — там этой страницы нет.
