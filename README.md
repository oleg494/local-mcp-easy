...
- **oauth** — OAuth 2.1 с Dynamic Client Registration и PKCE (`S256`) для любых OAuth MCP-клиентов;
- **legacy** — статический Bearer token (классическое поведение 1.x, по умолчанию);
- **dual** — оба варианта одновременно на одном `/mcp`.
...
```text
legacy  — только статический Bearer token (по умолчанию; например Notion, как в линии 1.x)
...
При смене публичного URL меняются issuer, discovery-ссылки, redirect-конфигурация и audience уже выданных токенов, поэтому launcher **блокирует** запуск `oauth`/`dual` на нестабильном URL. Разрешённые варианты:
...
Переменные тонкой настройки: `MCP_OAUTH_ACCESS_TTL` (сек, по умолчанию 3600), `MCP_OAUTH_REFRESH_TTL` (по умолчанию 30 дней), `MCP_OAUTH_MAX_CLIENTS` (100), `MCP_OAUTH_UNUSED_CLIENT_TTL` (3600), `MCP_OAUTH_CONSENT_MAX_ATTEMPTS` (на транзакцию, 5), `MCP_OAUTH_CONSENT_FAILURE_WINDOW_SECONDS` (60), `MCP_OAUTH_CONSENT_MAX_FAILURES` (10).
...
- оригинальный проект `notion-local-mcp-easy` Олега Алёшина;
